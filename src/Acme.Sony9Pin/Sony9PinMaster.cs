...
using Acme.Sony9Pin.CommandBlocks.SenseRequest;
using Acme.Sony9Pin.CommandBlocks.SenseReturn;
using Acme.Sony9Pin.CommandBlocks.StatusData;
using Acme.Sony9Pin.CommandBlocks.SystemControl;
...
namespace Acme.Sony9Pin
{
    ...
    ///     insure that no more than 10 msec lapses between bytes in a Cmd1 block. The master must immediatly stop sending
    ///     data when it receives a NAK + Error Data message. If the Error Data contains "Undefined Cmd1" the master may
    ///     immediatley send another Cmd1, otherwise it must wait at least 10 msec before sending another Cmd1. When
    ...
    public class Sony9PinMaster : Sony9Pin
    {
        ...
        private readonly AutoResetEvent _workerThreadStopped = new AutoResetEvent(false);
        ...
        /// <summary>
        /// </summary>
        private StatusData _statusData = new StatusData();
        ...
        public override void Close()
        {
            ...
            if (null != _queueReaderWorker && _queueReaderWorker.WorkerSupportsCancellation)
            {
        ...
        {
            Command(CommandBlock.ToCmd1DataCount(cmd1, dataCount), cmd2, new byte[0]);
        }
        ...
        {
            // do not pass 'null' as last parameter, but 'new byte[0]' to prevent errors
            Command(cmd1DataCount, cmd2, new byte[0]);
        }
        ...
        protected void Command(byte cmd1DataCount, byte cmd2, byte[] data)
        {
            ...
            if ((null != _commandBlocks) && (!_commandBlocks.IsAddingCompleted))
            {
        ...
        private void QueueReader(object sender, DoWorkEventArgs e)
        {
            ...
            while (!_commandBlocks.IsAddingCompleted)
            {
                ...
                try
                {
                    ...
                    while (Serial.IsOpen)
                    {
                        ...
                        try
                        {
                            ...
                            // Remove any unread charaters from the serial buffers
                ...
                {                    
                    // Oei - a character couldnt be read within the time given.
        ...
        static int  _currentTimeSenseOrStatusSense;
